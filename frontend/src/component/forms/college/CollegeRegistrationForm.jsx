/**
 * CollegeRegistrationForm.jsx
 * Fixes:
 *  1. Close button moved to sticky dark header — never overlaps content
 *  2. Form no longer resets on step change (removed key={step} & per-step resolver)
 *  3. Submit actually fires — manual Yup validate + fetch with proper error handling
 *
...
// ─── Import constants from your file ─────────────────────────────────────────
import {
  COLLEGE_TYPES,
  INDIA_STATES,
} from "../../../constant/collegeRegistration";

const STEPS = ["Basic Info", "Address", "Documents", "Courses", "Payment"];
const ACCEPTED_IMAGE_TYPES = ".jpg,.jpeg,.png";
const ACCEPTED_DOC_TYPES   = ".jpg,.jpeg,.png,.pdf";
...
const optionalFile = (label) =>
  Yup.mixed().nullable()
    .test("fileSize", `${label} must be under ${MAX_FILE_SIZE_MB} MB`,
      (v) => !v || v.size <= MAX_FILE_SIZE_BYTES);
...
const basicInfoSchema = Yup.object({
  collegeName: Yup.string().trim()
    .min(3, "Min 3 characters").max(150, "Max 150 characters")
    .required("College name is required"),
  collegeCode: Yup.string().trim()
    .min(2, "Min 2 characters").max(20, "Max 20 characters")
    .matches(/^[A-Z0-9]+$/, "Uppercase letters and numbers only")
  ...
  website: Yup.string().url("Must be a valid URL (include https://)")
    .optional().nullable().transform((v) => v === "" ? null : v),
  establishedYear: Yup.number()
    .typeError("Enter a valid year")
    .integer().min(1800, "Min 1800").max(new Date().getFullYear(), `Max ${new Date().getFullYear()}`)
    .optional().nullable().transform((val, orig) => orig === "" ? null : val),
  collegeType: Yup.string()
    .oneOf(COLLEGE_TYPES, "Select a valid type").required("College type is required"),
  affiliation: Yup.string().max(150, "Max 150 characters").optional(),
});

const addressSchema = Yup.object({
  address: Yup.object({
    street:  Yup.string().trim().min(5, "Min 5 characters").required("Street is required"),
    city:    Yup.string().trim().min(2, "Min 2 characters").required("City is required"),
...
export const courseItemSchema = Yup.object({
  ...
  fees:        Yup.number().typeError("Must be a number")
    .min(0, "Cannot be negative").max(10000000, "Value too high").required("Fees is required"),
...
const paymentSchema = Yup.object({
  paymentReceipt: requiredFile("Payment receipt"),
});
...
const C = {
  ...
  green:   "#16a34a",
  greenBg: "#f0fdf4",
  shadowMd: "0 8px 40px rgba(15,32,68,0.18)",
};
...
const mkInp = (hasErr) => ({
  ...
  borderRadius: 9, fontSize: 14, fontFamily: font.body,
  color: C.navy, background: hasErr ? C.redBg : C.white,
  outline: "none", boxSizing: "border-box", transition: "border-color 0.15s, box-shadow 0.15s",
});

const onFocus = (e, hasErr) => {
  e.target.style.borderColor = hasErr ? C.red : C.gold;
  e.target.style.boxShadow   = `0 0 0 3px ${hasErr ? "rgba(220,38,38,0.12)" : "rgba(201,151,58,0.18)"}`;
};
const onBlur = (e, hasErr) => {
  e.target.style.borderColor = hasErr ? C.red : C.border;
  ...
  return (
    <Field label={label} required={required} error={err} hint={hint} span={span}>
      <input {...register(name)} type={type} placeholder={placeholder}
        ...
        onFocus={(e) => onFocus(e, !!err)} onBlur={(e) => onBlur(e, !!err)} {...rest} />
    </Field>
  );
}
  ...
  return (
    <Field label={label} required={required} error={err} hint={hint} span={span}>
      <select {...register(name)}
        ...
        onFocus={(e) => onFocus(e, !!err)} onBlur={(e) => onBlur(e, !!err)}>
      ...
      <div onClick={() => ref.current.click()}
        ...
        <div>
          ...
          <div style={{ fontSize: 11.5, color: C.slateXl, marginTop: 1, fontFamily: font.body }}>
            {accept.replace(/\./g,"").toUpperCase().split(",").join(", ")} · max {MAX_FILE_SIZE_MB} MB
          </div>
        </div>
      </div>
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 16 }}>
      ...
      <RHFInput name="collegeCode" label="College Code" required
        ...
        xStyle={{ textTransform: "uppercase" }}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      <div style={{ padding: "11px 14px", background: "#fffbeb",
        ...
        📋 Upload scanned copies. Max {MAX_FILE_SIZE_MB} MB per file.
      </div>
      <FileField label="College Logo" name="logo" accept={ACCEPTED_IMAGE_TYPES}
      ...
      <FileField label="Affiliation Certificate" name="affiliationCert" required
        accept={ACCEPTED_DOC_TYPES} value={files.affiliationCert} onChange={onFileChange}
      ...
      <FileField label="Registration Certificate" name="registrationCert"
        accept={ACCEPTED_DOC_TYPES} value={files.registrationCert} onChange={onFileChange}
  ...
  const DI = ({ field, label, placeholder, type = "text", required }) => (
    <Field label={label} required={required} error={draftErr[field]}>
      <input type={type} placeholder={placeholder} value={draft[field]}
        ...
        onFocus={e => onFocus(e, !!draftErr[field])}
        onBlur={e => onBlur(e, !!draftErr[field])} />
    </Field>
  );
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      {fields.length > 0 && (
        <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
          {fields.map((field, i) => (
            <div key={field.id} style={{ border: `1px solid ${C.border}`, borderRadius: 10,
              ...
              <button type="button" onClick={() => remove(i)} style={{
                ...
                padding: "3px 8px", fontFamily: font.body, fontWeight: 600 }}>
                Remove
              </button>
              <div style={{ fontSize: 14, fontWeight: 700, color: C.navy,
                ...
                <span style={{ fontSize: 11, fontWeight: 600, background: C.navyLt,
                  padding: "2px 7px", borderRadius: 4, marginLeft: 8, color: C.navyMid,
                  fontFamily: "monospace" }}>
                  {field.courseCode}
                </span>
              </div>
              <div style={{ fontSize: 12.5, color: C.slate, marginTop: 3, fontFamily: font.body }}>
                {[field.duration && `Duration: ${field.duration}`,
                  `Seats: ${field.totalSeats}`,
                  `Fees: ₹${Number(field.fees).toLocaleString("en-IN")}`
                ].filter(Boolean).join("  ·  ")}
              </div>
            </div>
          ))}
      ...
      {!adding ? (
        ...
        <div style={{ border: `1px solid ${C.border}`, borderRadius: 11, padding: 18,
          background: C.white, boxShadow: "0 4px 16px rgba(15,32,68,0.07)" }}>
          <p style={{ margin: "0 0 14px", fontSize: 14, fontWeight: 700,
            color: C.navy, fontFamily: font.body }}>New Course</p>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 13 }}>
            ...
            <div style={{ gridColumn: "1/-1" }}>
              <Field label="Description" error={draftErr.description}>
                <textarea style={{ ...mkInp(!!draftErr.description), height: 72,
                  resize: "none", paddingTop: 10, lineHeight: 1.5 }}
                  value={draft.description} onChange={e => setD("description", e.target.value)}
                  placeholder="Brief description (max 500 chars)…"
                  onFocus={e => onFocus(e, !!draftErr.description)}
                  onBlur={e => onBlur(e, !!draftErr.description)} />
              </Field>
            </div>
          </div>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      <div style={{ padding: 18, background: C.cream, border: `1px solid ${C.border}`,
        ...
        <p style={{ margin: "0 0 12px", fontSize: 14, fontWeight: 700,
          color: C.navy, fontFamily: font.body }}>Scan QR to Pay Registration Fee</p>
      ...
      <div style={{ padding: 18, background: C.white, border: `1px solid ${C.border}`, borderRadius: 12 }}>
        <p style={{ margin: "0 0 12px", fontSize: 14, fontWeight: 700,
          color: C.navy, fontFamily: font.body }}>Upload Payment Proof</p>
        <FileField label="Payment Receipt / Screenshot" name="paymentReceipt" required
          accept={ACCEPTED_DOC_TYPES} value={files.paymentReceipt}
        ...
        return (
          <div key={i} style={{ display: "flex", alignItems: "center",
            ...
            <div style={{ display: "flex", flexDirection: "column",
              alignItems: "center", gap: 5, minWidth: 50 }}>
              <div style={{
                ...
                boxShadow: active ? "0 0 0 4px rgba(201,151,58,0.18)" : "none",
                transition: "all 0.2s",
              }}>
                {done ? "✓" : i + 1}
              </div>
              <span style={{ fontSize: 10, fontWeight: active ? 700 : 500,
                ...
                color: active ? C.navy : done ? C.gold : C.slateXl,
                whiteSpace: "nowrap" }}>
                {label}
              </span>
            </div>
            {i < STEPS.length - 1 && (
              <div style={{ flex: 1, height: 2, minWidth: 16, marginBottom: 20,
                background: done ? C.gold : C.border, transition: "background 0.2s" }} />
            )}
          </div>
        );
      })}
...
export default function CollegeRegistrationForm({ onClose }) {
  ...
  // No per-step resolver — values persist across step changes
const methods = useForm({
  ...
  // Validate file-only steps against Yup
  ...
  // Validate RHF steps via Yup, push errors back into RHF
  const validateRHF = async () => {
    try {
      await STEP_SCHEMAS[step].validate(getValues(), { abortEarly: false });
      return true;
    } catch (err) {
      err.inner.forEach(e => setError(e.path, { type: "manual", message: e.message }));
      return false;
    }
  };
  ...
  const handleNext = async () => {
    ...
    if (step === 2 || step === 4) {
      ...
      // touch fields first so errors render, then Yup validate
      await trigger(STEP_RHF_FIELDS[step]);
  ...
  const doSubmit = async (data) => {
    ...
    try {
      ...
      const BASE = import.meta.env.VITE_API_URL
        ...
        || "";
      const res  = await fetch(`${BASE}/colleges/register`, { method: "POST", body: formData });
      const json = await res.json();
      if (!res.ok || !json.success) throw new Error(json.message || `Error ${res.status}`);
      setSuccess(true);
    } catch (e) {
      setFileErrors(prev => ({ ...prev, _submit: e.message || "Submission failed. Try again." }));
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <Backdrop onClose={onClose}>
        <div style={{ background: C.white, borderRadius: 18, padding: "48px 40px",
          maxWidth: 440, width: "100%", textAlign: "center", boxShadow: C.shadowMd }}>
          ...
          <h2 style={{ fontFamily: font.display, fontSize: 24, color: C.navy,
            margin: "0 0 10px", fontWeight: 400 }}>Registration Submitted!</h2>
          <p style={{ fontSize: 14, color: C.slate, lineHeight: 1.75,
            margin: "0 0 26px", fontFamily: font.body }}>
            Your application for{" "}
            <strong style={{ color: C.navy }}>{getValues("collegeName")}</strong>{" "}
            with {getValues("courses")?.length || 0} course(s) has been received.
            <br />Our team will review and verify payment.
          </p>
  ...
  return (
    <Backdrop onClose={onClose} zIndex={10001}>
      ...
      <div style={{
        position: "relative", width: "100%", maxWidth: 700, maxHeight: "96vh",
        background: C.white, borderRadius: 18, boxShadow: C.shadowMd,
        display: "flex", flexDirection: "column", overflow: "hidden",
      }}> 

        {/* ── FIXED HEADER with close button — never overlaps body ── */}
        <div style={{
          ...
          <div style={{ minWidth: 0 }}>
            ...
            <p style={{ fontSize: 12, color: "rgba(255,255,255,0.55)",
              ...
              Complete all {STEPS.length} steps to apply for listing
            </p>
          </div>
          {/* Close — in header row, never overlaps scrollable content */}
        ...
        <div className="cd-scroll" style={{ flex: 1, overflowY: "auto", padding: "24px 28px 28px" }}>
          ...
          {/* Form — NO key={step} so values persist */}
          <FormProvider {...methods}>
            <form onSubmit={handleSubmit(doSubmit)} noValidate>
              ...
              <div style={{ display: "flex", justifyContent: "space-between",
                marginTop: 24, gap: 10, paddingTop: 18,
                ...
                {step < STEPS.length - 1 ? (
                  ...
                  <button type="submit" disabled={loading} className="cd-nav"
                    style={{ height: 42, padding: "0 24px", border: "none",
                      ...
                      background: loading ? C.slateXl
                        : `linear-gradient(135deg, ${C.navy}, ${C.navyMid})`,
                      color: C.white, fontSize: 13.5, fontWeight: 700,
                      cursor: loading ? "not-allowed" : "pointer",
                      boxShadow: loading ? "none" : "0 4px 14px rgba(15,32,68,0.28)",
                      display: "flex", alignItems: "center", gap: 7 }}>
                    {loading
                      ? <><span style={{ width: 13, height: 13, border: "2px solid rgba(255,255,255,0.35)",
  ...
  return (
    <div
      style={{ position: "fixed", inset: 0, zIndex,
        background: "rgba(10,20,44,0.6)", backdropFilter: "blur(4px)",
        display: "flex", alignItems: "center", justifyContent: "center",
